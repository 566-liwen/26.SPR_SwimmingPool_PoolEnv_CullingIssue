    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            */
			...
			{   // d1 color (wave to camera)             
                half toColor = i.dist/30;
				GRT0 = fixed4(toColor, toColor, toColor,1);
                // normal

                //float3 ddxPos = ddx(i.vertex);
                //float3 ddyPos = ddy(i.vertex)  * _ProjectionParams.x;
                //float3 normal = normalize( cross(ddxPos, ddyPos));
                //GRT1 = fixed4(normal.x, normal.y, normal.z, 1);
                //GRT1 = fixed4(i.normals.x, i.normals.y, i.normals.z, 1);
                GRT1 = fixed4(i.normal, 1);
			}
            ENDCG
        }
    }
